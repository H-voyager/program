...
int main()
{ 
	//在我的理解中软件的开发流程只要分为以下几个步骤

	//首先是需求的确认。确认软件需要完成的功能，这些功能的可执行性和开发环境，开发语言，平台的确认
	
	//然后是程序架构的搭建，这一部分主要由架构师等完成整个软件系统 的框架，功能，接口，模块之间的相互联系等的设计。
	
	//其次就是接口，内在功能，模块的具体实现，这一部分也是整个软件设计工程中的一个任务量比较大的部分。
	
	//之后就是测试环节，在经过开发人员的bug简单调试自测之后，在公司的生产实际中应该要进行大量的系统化的专业测试。\
		这一部分应该是由专业的测开人员完成，之后在再开发人员的沟通。
	
	//最终就是软件的部署，进行生产实际的应用和维护。
	//以上就是我所理解的一个公司完成软件产品的大致流程。

	//任意位数的PI值计算，此处设置3000位，这个可以使用宏进行替换
